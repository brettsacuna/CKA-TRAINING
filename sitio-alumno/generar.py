# -*- coding: utf-8 -*-
"""
Genera la web estática del alumno (para GitHub Pages) a partir del contenido
del repositorio.

Salida: ../docs/  (GitHub Pages -> rama main, carpeta /docs)

Incluye: índice, guía del curso, páginas de clase, laboratorios (markdown
renderizado), visor de recursos (YAML/scripts) y, en la Clase 6, checklist y
cheatsheet. NO incluye las carpetas SOLUCIONES/.
...
OUT = ROOT / "docs"
SITE = "CKA Hands-On Training"
BASE_TAGLINE = "Material del alumno · Kubernetes práctico para el examen CKA"
...
def rewrite_links(body, *, in_class):
    ...
    def sub_href(m):
        ...
        if re.match(r"RECURSOS/(YAML|SCRIPTS)/?$", h):
...
def shell(*, title, root, breadcrumb, content, nav_active="", head_extra="",
    ...
    nav = (na("inicio", "Inicio", f"{root}index.html")
           + na("clases", "Clases", f"{root}guia.html")
           + na("cheat", "Cheatsheet", f"{root}clase-6/cheatsheet.html"))
    crumbs = ""
    if breadcrumb:
        parts = []
...
<header class="topbar">
  <a class="brand" href="{root}index.html">
    ...
    <span>CKA HANDS-ON TRAINING</span>
  </a>
...
<footer>
  <span>{SITE} · material del alumno</span>
...
def resources_of(src_dir):
    res = []
    for sub in ("YAML", "SCRIPTS"):
        d = src_dir / "RECURSOS" / sub
        if d.is_dir():
...
def build():
    ...
    for i, (slug, src, n, title, sub, icon) in enumerate(CLASES):
        ...
        # extras de la clase 6
        if slug == "clase-6":
            for fname, page in (("02-CHECKLIST-CKA.md", "checklist.html"),
                                ("03-CHEATSHEET-CKA.md", "cheatsheet.html")):
                fp = src_dir / fname
                if fp.exists():
                    body = process_md(fp.read_text(encoding="utf-8"), in_class=True)
                    label = "Checklist CKA" if "checklist" in page else "Cheat Sheet CKA"
                    content = f'<article class="md wide">{body}</article>'
                    (cdir / page).write_text(shell(
                        title=f"{label} · Clase 6", root="../",
                        breadcrumb=[("Inicio", "../index.html"),
                                    ("Clase 6", "index.html"), (label, None)],
                        content=content, nav_active="cheat" if "cheat" in page else "clases"),
                        encoding="utf-8")
    ...
    print(f"docs/ generado — {n_pages} páginas HTML")
...
def build_index():
    ...
    for slug, src, n, title, sub, icon in CLASES:
      ...
      <a class="ccard" href="{slug}/">
        <div class="ccard-top"><span class="mono num">{int(n):02d}</span>{svg(ICONS[icon], 26)}</div>
        <h3>{html.escape(title)}</h3>
        <p>{html.escape(sub)}</p>
        <div class="ccard-foot"><span class="mono">180 min · {nlabs} labs</span><span class="go">Ver clase →</span></div>
      </a>""")
    content = f"""
<section class="hero">
  <div class="eyebrow">Material del alumno</div>
  <h1>CKA Hands-On Training</h1>
  <p class="lead">Entrenamiento práctico de administración de Kubernetes orientado al examen
  <strong>CKA</strong>. Seis clases de tres horas, con laboratorios de dificultad progresiva
  y escenarios de troubleshooting.</p>
  <div class="stats">
    <div class="stat"><b>18 h</b><span>Duración</span></div>
    <div class="stat"><b>6 clases</b><span>Módulos</span></div>
    <div class="stat"><b>25</b><span>Laboratorios</span></div>
    <div class="stat"><b>70 %</b><span>Práctica</span></div>
  </div>
</section>

<section class="block">
  <h2 class="section-title">Las seis clases</h2>
  <div class="cgrid">{''.join(cards)}</div>
</section>

<section class="block">
  <div class="method">
    <span class="method-k">Método</span>
    <span class="mono method-flow">EXPLICAR → DEMOSTRAR → PRACTICAR → ROMPER → DIAGNOSTICAR → CORREGIR → VALIDAR</span>
  </div>
  <p class="notice">
    {svg('<circle cx="12" cy="12" r="9"></circle><path d="M12 8v5M12 16h.01"></path>', 15, "#8493AD", "2")}
    Las carpetas <span class="mono">SOLUCIONES/</span> son material del instructor y no se publican en este sitio.
    <a href="guia.html">Guía completa del curso →</a>
  </p>
</section>
"""
...
def build_guia():
    body = process_md((ROOT / "README.md").read_text(encoding="utf-8"), in_class=False)
    content = f'<article class="md wide">{body}</article>'
    (OUT / "guia.html").write_text(shell(
        title="Guía del curso", root="", nav_active="clases",
        breadcrumb=[("Inicio", "index.html"), ("Guía del curso", None)],
...
def build_clase(slug, src, n, title, sub, icon, src_dir, labs, res, prev_, next_):
    ...
    def res_rows(kind):
        items = [p for k, p in res if k == kind]
        return "".join(
            f'<div class="rrow mono"><span>{p.name}</span>'
            f'<span class="acts"><a href="recursos.html#{p.name}" title="Ver">{svg(IC_COPY,14,"currentColor","2")}</a>'
            f'<a href="recursos/{p.name}" download title="Descargar">{svg(IC_DL,14,"currentColor","2")}</a></span></div>'
            for p in items)

    extra_rail = ""
    if slug == "clase-6":
        extra_rail = ('<div class="railcard"><div class="railcard-h">Material adicional</div>'
                      '<a class="rlink" href="checklist.html">Checklist CKA</a>'
                      '<a class="rlink" href="cheatsheet.html">Cheat Sheet CKA</a></div>')

    prevlink = f'<a href="../{prev_[0]}/">← Clase {prev_[2]}</a>' if prev_ else '<span></span>'
    nextlink = f'<a href="../{next_[0]}/">Clase {next_[2]} →</a>' if next_ else '<span></span>'

    content = f"""
<section class="clase-head">
  <div class="eyebrow">Clase {n}</div>
...
<div class="two-col">
  ...
  <aside class="rail">
    ...
    <div class="railcard">
      <div class="railcard-h">Recursos · YAML</div>
      <div class="rlist">{res_rows("YAML")}</div>
      <div class="railcard-h" style="margin-top:16px">Scripts</div>
      <div class="rlist">{res_rows("SCRIPTS")}</div>
      <a class="btn" href="recursos.html">Abrir visor de recursos</a>
    </div>
    {extra_rail}
  </aside>
</div>
...
"""
    (OUT / slug / "index.html").write_text(shell(
        title=f"Clase {n} — {title}", root="../", nav_active="clases",
        breadcrumb=[("Inicio", "../index.html"), (f"Clase {n}", None)],
        content=content), encoding="utf-8")
...
<div class="lab-layout">
  <aside class="lab-side">
    <details class="lab-nav" open>
      <summary>Clase {n} · Laboratorios</summary>
      <div class="railcard-h">Clase {n} · Laboratorios</div>
...
"""
    (ldir / "index.html").write_text(shell(
        title=f"LAB {n}.{m['n']} — {m['title']}", root="../../", nav_active="clases",
        breadcrumb=[("Inicio", "../../index.html"),
                    (f"Clase {n}", "../index.html"),
                    (f"LAB {n}.{m['n']}", None)],
        content=content, body_class="labpage"), encoding="utf-8")


def build_recursos(slug, src, n, title, res):
    files = [p for _, p in res]
    listing = []
    panels = []
    last_kind = None
    for kind, p in res:
        if kind != last_kind:
            ...
            last_kind = kind
        listing.append(
            f'<a class="rl-item mono" href="#{p.name}" data-file="{p.name}">{p.name}</a>')
    for kind, p in res:
        ...
        nlines = raw.count("\n") + 1
        panels.append(f"""
    <div class="viewer" data-file="{p.name}" hidden>
      <div class="viewer-h">
        <div class="vh-l"><span class="mono fn">{p.name}</span>
          <span class="badge mono">{p.suffix.lstrip('.').upper() or 'TXT'}</span>
          <span class="muted">{nlines} líneas · {p.stat().st_size} B</span></div>
        <div class="vh-r">
          <button class="btn sm copybtn" data-target="{p.name}">{svg(IC_COPY,14,"currentColor","2")} Copiar</button>
          <a class="btn sm primary" href="recursos/{p.name}" download>{svg(IC_DL,14,"currentColor","2")} Descargar</a>
        </div>
      </div>
...
<section class="clase-head compact">
  <div class="eyebrow">Clase {n} · Recursos</div>
  <h1>Manifiestos y scripts</h1>
  <p class="lead">Archivos de referencia de los laboratorios de la clase. Cópialos o descárgalos.</p>
</section>
...
"""
    (OUT / slug / "recursos.html").write_text(shell(
        title=f"Recursos · Clase {n}", root="../", nav_active="clases",
        breadcrumb=[("Inicio", "../index.html"), (f"Clase {n}", "index.html"),
                    ("Recursos", None)],
...
.rrow{display:flex;align-items:center;justify-content:space-between;font-size:12.5px;padding:6px 0;color:var(--body)}
.rrow .acts{display:flex;gap:10px;color:var(--faint)}
